...
-- ============================================
-- 🎮 MAIN GUI CREATION (HYBRID VERSION)
-- ============================================
local ScreenGui = Instance.new("ScreenGui")
...
local function createRightsPage()
    ...
    VersionInfo.Text = "الإصدار: 6.0.0 ULTIMATE HYBRID"
...
-- Function to find and use HDAdmin
local function FindHDAdmin()
    local rs = game:GetService("ReplicatedStorage")
    if rs:FindFirstChild("HDAdminHDClient") then
        return rs.HDAdminHDClient
    end
    
    -- Try to find in other places
    for _, obj in pairs(rs:GetChildren()) do
        if obj.Name:lower():find("hdadmin") then
            return obj
        end
    end
...
local function ExecuteCommand(command)
    local hdadmin = FindHDAdmin()
    if hdadmin and hdadmin:FindFirstChild("Signals") then
        local signals = hdadmin.Signals
        if signals:FindFirstChild("RequestCommandSilent") then
            signals.RequestCommandSilent:InvokeServer(command)
            return true
        end
    end
    return false
...
local function startCopyStrong()
    ...
    while copyStrongActive and targetPlayer do
        ...
        local command = ". .neon "..prefix.." .paint "..prefix.." pk .size "..prefix.." 3 .height "..prefix.." 0.1 .aura "..prefix.." .title "..prefix.." تم النسخ بواسطة Z7F"
...
local function startCopyFunny()
    ...
    while copyFunnyActive and targetPlayer do
        ...
        local command = ".dog "..prefix.." .jp "..prefix.." 0 .titlepk "..prefix.." \218\175\217\132\216\168 \217\133\216\183\217\139\217\138\216\185\217\139"
...
local function startCopyStrong2()
    ...
    while copyStrong2Active and targetPlayer do
        local plrName = targetPlayer.Name
        local prefix = #plrName >= 5 and string.sub(plrName,1,3) or plrName
        local command = ".size "..prefix.." 3 .fat "..prefix.." .box "..prefix.." .sit "..prefix.." .neon "..prefix.." .paint "..prefix.." pk .shine "..prefix.." .titlepk "..prefix.." \217\133\217\139\217\137\216\175\217\139\216\185\217\137\217\136\217\139\216\179\217\145"
...
local function startCopyBeautiful()
    ...
    while copyBeautifulActive and targetPlayer do
        ...
        local command = ". .size "..prefix.." 3 .height "..prefix.." 0.1 .neon "..prefix.." .paint "..prefix.." pk .sit "..prefix.." .jp "..prefix.." 0 .aura "..prefix.." .fat "..prefix.." .titlepk "..prefix.." Z7F \217\129\216\173\217\139\217\137\217\132\217\139\217\138\217\139\217\139\217\139"
...
local function startCopyExtend()
    ...
    while copyExtendActive and targetPlayer do
        ...
        local command = ". .char "..prefix.." .neon "..prefix.." .size "..prefix.." 3 .sit "..prefix.." .jp "..prefix.." 0 .height "..prefix.." 3 .fast "..prefix
...
local function startCopyFat()
    ...
    while copyFatActive and targetPlayer do
        ...
        local command = ". .char "..prefix.." ardaomeroglucooj .neon "..prefix.." .paint "..prefix.." pk .sit "..prefix.." .jp "..prefix.." 0 .fat "..prefix.." .thin "..prefix
...
local function startCopyThin()
    ...
    while copyThinActive and targetPlayer do
        ...
        local command = ".neon "..prefix.." .fast "..prefix.." .size "..prefix.." 2 .paint "..prefix.." pk .sit "..prefix.." .thin "..prefix.." .shine "..prefix
...
local function startCopyAntiHack()
    ...
    while copyAntiHackActive and targetPlayer do
        ...
        local command = ".size "..prefix.." 3 .height "..prefix.." 1 .fat "..prefix.." .thin "..prefix.." .sit "..prefix.." .neon "..prefix.." .paint "..prefix.." pk"
...
local function startDoubleRe()
    ...
    while doubleReActive and targetPlayer do
        ...
        local command = ".re "..prefix.." .re "..prefix
...
local function startDoubleAura()
    ...
    while doubleAuraActive and targetPlayer do
        ...
        local command = ".aura "..prefix.." .aura "..prefix
...
AntiCopyButton.MouseButton1Click:Connect(function()
    ...
    if antiCopyActive then
        ...
        createNotification("Z7F", "تم تشغيل مضاد النسخ المتقدم", "rbxassetid://7992557358", 3)
    else
        AntiCopyButton.Text = "مضاد النسخ: ❌"
        ...
        createNotification("Z7F", "تم إيقاف مضاد النسخ", "rbxassetid://7992557358", 3)
    end
end)
...
AntiKillButton.MouseButton1Click:Connect(function()
    ...
    if antiKillActive then
        ...
        createNotification("Z7F", "تم تشغيل مضاد القتل", "rbxassetid://7992557358", 3)
    else
        AntiKillButton.Text = "مضاد القتل: ❌"
        ...
        createNotification("Z7F", "تم إيقاف مضاد القتل", "rbxassetid://7992557358", 3)
    end
end)
...
createNotification("Z7F", "Z7F HYBRID - النسخة المتطورة", "rbxassetid://7992557358", 5)
...
local function cleanup()
    ...
    createNotification("Z7F", "تم تنظيف السكربت", "rbxassetid://7992557358", 2)
end
...
task.spawn(function()
    ...
    pcall(function()
        ...
        if events then
            ...
            if sendMessage then
                sendMessage:FireServer("Z7F HYBRID ON TOP")
            end
        end
    end)
end)
...
task.spawn(function()
    ...
    pcall(function()
        ...
        if events then
            local sendMessage = events:FindFirstChild("SendMessage")
            if sendMessage then
                sendMessage:FireServer("Z7F~THE~BEST~HYBRID~SCRIPT")
            end
        end
    end)
end)
    ...
    local function sendThirdMessage()
        pcall(function()
            ...
            if events then
                local sendMessage = events:FindFirstChild("SendMessage")
                if sendMessage then
                    sendMessage:FireServer("#ScRiPt ! Z7F ! HYBRID#")
                end
            end
        end)
    end
    ...
    local function sendFourthMessage()
        pcall(function()
            ...
            if events then
                local sendMessage = events:FindFirstChild("SendMessage")
                if sendMessage then
                    sendMessage:FireServer("<iUse-Script-Z7F-HYBRID>by Z7F")
                end
            end
        end)
    end
